    ...
    if hasattr(shap.explainers._tree, 'XGBTreeModelLoader'):
        ...
        if not hasattr(XGBTreeModelLoader, '_original_init'):
            ...
            def patched_xgb_init(self, model):
                """Patched XGBTreeModelLoader init to fix base_score parsing for XGBoost 3.1.0+ compatibility."""
                # The issue: XGBoost 3.1.0+ stores base_score as '[5E-1]' instead of '0.5'
                # SHAP's code at line 2104 does: self.base_score = float(learner_model_param["base_score"])
                # This fails because float('[5E-1]') raises ValueError
                # Solution: Fix the booster config BEFORE calling original_init, ensuring the fix persists
                ...
                if 'learner' in config_dict and 'learner_model_param' in config_dict['learner']:
                    base_score_param = config_dict['learner']['learner_model_param'].get('base_score', None)
                ...
                if base_score_needs_fix:
                    ...
                    try:
                        ...
                        verify_base = verify_dict.get('learner', {}).get('learner_model_param', {}).get('base_score', '')
                        ...
                        if isinstance(verify_base, str) and verify_base.startswith('['):
                            ...
                            verify_base2 = verify_dict2.get('learner', {}).get('learner_model_param', {}).get('base_score', '')
                            ...
                            if isinstance(verify_base2, str) and verify_base2.startswith('['):
                                ...
                                def fixed_save_config():
                                    ...
                                    if 'learner' in cfg and 'learner_model_param' in cfg['learner']:
                                        cfg['learner']['learner_model_param']['base_score'] = str(base_score_float)
                ...
                # This ensures that even if SHAP reads config multiple times, it gets the fixed version
...
def load_shap_explainer(_model, background_data):
    ...
    if isinstance(_model, Pipeline):
        ...
        for step_name, step_obj in _model.steps:
            ...
            if any(xgb_type in step_type_str for xgb_type in ['xgb', 'lightgbm', 'catboost', 'randomforest', 'decisiontree', 'gradientboosting']):
    ...
    try:
        import xgboost as xgb
        is_xgboost_model = isinstance(model_to_use, xgb.XGBClassifier) or isinstance(model_to_use, xgb.XGBRegressor)
    except ImportError:
        is_xgboost_model = False
    
    is_tree_model = is_xgboost_model or any(xgb_type in model_type_str for xgb_type in ['xgb', 'lightgbm', 'catboost', 'randomforest', 'decisiontree', 'gradientboosting'])
    ...
    try:
        ...
        explainer_creation_method = "direct_model"
        
        if is_xgboost_model:
            # Fix base_score attribute if it's stored as a string (XGBoost version compatibility issue)
            ...
            try:
                if hasattr(model_to_use, 'get_booster'):
                    ...
                    if 'learner' in config_dict and 'learner_model_param' in config_dict['learner']:
                        base_score_param = config_dict['learner']['learner_model_param'].get('base_score', None)
                        if base_score_param and isinstance(base_score_param, str) and base_score_param.startswith('['):
                    ...
                    explainer_creation_method = "get_booster_fixed" if base_score_fixed else "get_booster"
            except Exception as e:
                # If fixing fails, try using booster directly or model itself
                try:
                    if hasattr(model_to_use, 'get_booster'):
                        model_for_explainer = model_to_use.get_booster()
                        explainer_creation_method = "get_booster_fallback"
                    elif hasattr(model_to_use, 'booster_') and model_to_use.booster_ is not None:
                        model_for_explainer = model_to_use.booster_
                        explainer_creation_method = "booster_attribute"
                    else:
                        model_for_explainer = model_to_use
                        explainer_creation_method = "direct_model_fallback"
                except Exception:
                    model_for_explainer = model_to_use
                    explainer_creation_method = "direct_model_fallback"
        ...
        if champion_model_obj is not None:
            ...
            if explainer is not None:
                ...
                if shap_error:
                    ...
                    error_msg += "SHAP explanations require a tree-based model (XGBoost, LightGBM, etc.). "
                    error_msg += "Ensure the champion model (xgboost_weighted_optimized.pkl) is an XGBoost model."
