...
def objective(trial, X, y):
    ...
    pipeline = ImbPipeline(
        [("smote", SMOTE(random_state=42, k_neighbors=5)), ("xgb", xgb.XGBClassifier(**param))]
    )
...
def run_optimization(n_trials=50):
    ...
    study.optimize(
        lambda trial: objective(trial, X_train, y_train), n_trials=n_trials, callbacks=[callback]
    )
...
def train_best_model(study, X_train, y_train, X_val, y_val):
    ...
    final_pipeline = ImbPipeline(
        [("smote", SMOTE(random_state=42)), ("xgb", xgb.XGBClassifier(**best_params))]
    )
